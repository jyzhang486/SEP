...
public class ReportServiceImpl implements ReportService {
    ...
    private void sendDirectRequests(ReportRequest request) {
        CompletableFuture<ExcelResponse> excelFuture = CompletableFuture.supplyAsync(() -> {
            ...
            ExcelResponse excelResponse = rs.postForEntity("http://localhost:8888/excel", request, ExcelResponse.class).getBody();
            return excelResponse;
        }).handle((res, ex) -> {
            if(ex != null) {
                log.error("Excel Generation Error (Sync) : e", ex);
        ...
        CompletableFuture<PDFResponse> pdfFuture = CompletableFuture.supplyAsync(() -> {
            RestTemplate rs = new RestTemplate();
            PDFResponse pdfResponse = rs.postForEntity("http://localhost:9999/pdf", request, PDFResponse.class).getBody();
            return pdfResponse;
        }).handle((res, ex) -> {
            if(ex != null) {
                log.error("PDF Generation Error (Sync) : e", ex);
